<script>
  ...
  let iconMap = {
    "File System": fileLogo,
    Overview: Overview,
    "VS Code": vsLogo,
    "Js Paint": "https://win98icons.alexmeub.com/icons/png/paint_old-0.png",
    Default: "https://win98icons.alexmeub.com/icons/png/notepad_file-2.png",
  };
  ...
  function sendThing(e) {
    ...
    if (
      ...
      e.target.classList.contains("startIconImg") === false
    ) {
      const clickedNodeIndex = [].indexOf.call(
        clickedEle.parentNode.children,
        clickedEle,
      );
      ...
      clickedEle =
        clickedEle.parentNode.parentNode.children[clickedNodeIndex + 2];
      // console.log(clickedEle);
    } else if (
      e.target.nodeName === "BUTTON" ||
      e.target.classList.contains("startMenuSidebar") ||
      e.target.classList.contains("startMenuSidebarText") ||
      e.target.classList.contains("startMenuSidebarSpan") ||
      e.target.classList.contains("menuBarStart") ||
      e.target.classList.contains("startIconImg")
    ) {
      return;
    }
  ...
  >
    <div class="startMenuSidebar" style="">
      <div class="startMenuSidebarText">
        <span class="startMenuSidebarSpan" style="font-weight:bold">Shmul</span
        >.dev
      </div>
    </div>
    <div
      style="height:100%; width:20px;position:absolute;left:34px;bottom:-3px;background: transparent;"
    >
      <img
        src={iconMap["Default"]}
        width="20px"
    ...
    <p style="margin-left:40px">About My Site</p>
    <p>My Hobbies</p>
    <p>Resume</p>
    <p>My Ideas</p>
    <p>About Me</p>
...
<div
  class="notAButton notMyButtonStyles"
  style="width: calc(100% + 3px);margin-right:-1px;position: fixed"
>
  <button
    class="menuBarStart"
  ...
  >
    <img
      ...
      alt="windows 98 style start icon"
    />
  ...
  <div class="minimizedItems">
    {#each currentWindows as window}
      <button
        id={"minButt" + window.replace(/\s/g, "")}
        on:click|capture|preventDefault={(event) => forward(event, window)}
        class:classes={window === $glowWindow}
        class="appMinimized"
      >
        <img
          src={findIcon(iconMap, window)}
          alt="{window} bottom bar icon"
  ...
  <div
    class="timeBox"
    style="margin-right: -6px;white-space: nowrap;display: flex;background: linear-gradient( 90deg, rgb(26 26 101), rgb(154 143 233) );"
  >
    <!--<img src="https://win98icons.alexmeub.com/icons/png/gears_tweakui_a-1.png" alt="hello alt" class="gears">-->
    <img
      src="https://win98icons.alexmeub.com/icons/png/loudspeaker_rays_green-0.png"
      alt="windows 98 sound icon"
    ...
    <b
      id="time"
      class="timeText"
      style="margin-bottom:10px;display:block;margin-top:4px;color:white;font-family: 'Apple Garamond Bold'"
    ></b>
  </div>
</div>
...
<style>
  ...
  .startMenuSidebar {
    ...
    left: 3px;
    bottom: -1px;
    background: linear-gradient(90deg, rgb(154 143 233), rgb(26 26 101));
  }
  .startMenuIcon {
    margin-left: -6px;
    margin-right: 6px;
  ...
  .classes {
    animation-name: glow, animatedgradient;
