...
<script>
  ...
  // Double-click via click events (dblclick doesn't fire on mobile touch)
  let lastTapTime = 0;
  let lastTapId = null;

  function dblTap(id, callback) {
    return () => {
      const now = Date.now();
      if (now - lastTapTime < 500 && lastTapId === id) {
        callback();
        lastTapTime = 0;
        lastTapId = null;
      } else {
...
<div
  class="window-body"
  style="justify-content: center;text-align: center;min-height: 250px;"
>
  <div class="centerText">
    {#each foldersList as folder}
      <div
        ...
        role="button"
        tabindex="0"
        on:click={dblTap(folder, () => goDeeper(folder))}
      >
        <img
          class="menuFileIcon"
          alt="windows 98 filesystem file button goes to {folder}"
          src="https://win98icons.alexmeub.com/icons/png/directory_closed-5.png"
        />
    ...
    {#each filesList as file}
      <div
        ...
        role="button"
        tabindex="0"
        on:click={dblTap(file, () => openItem(file))}
      >
        <img
          class="menuFileIcon"
