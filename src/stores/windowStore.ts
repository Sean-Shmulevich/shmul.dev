...
function createWindowsStore() {
  const { subscribe, set, update } = writable<Record<string, WindowState>>({});

  let fileSysWindowsInternal: Record<string, number> = {}; // Internal state for sub-file windows
  let numFileWinInternal = 1; // Internal counter for sub-file windows

  return {
    subscribe,
    openWindow: (name: string, initialPosition?: Position) =>
      update((windows) => {
        if (!windows[name]) {
          windows[name] = {
            zIndex: 0,
    ...
    minimizeWindow: (name: string, minimized: boolean) =>
      update((windows) => {
        if (windows[name]) {
    ...
    openSubFileWindow: (name: string, currentMaxZIndex: number) =>
      update((windows) => {
        ...
        if (Object.keys(fileSysWindowsInternal).length < maxWindows) {
          ...
          if (!windows[name]) {
            // Only open if not already in store
            windows[name] = {
              zIndex: currentMaxZIndex + 1, // Set z-index to highest + 1
              minimized: false,
              position: {
                x: 180 + numFileWinInternal * 30,
                y: 100 + numFileWinInternal * 30,
              }, // Example: sequential positioning
            };
          } else {
